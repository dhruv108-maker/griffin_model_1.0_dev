...
class GriffinResultBuilder:
    ...
    def build(
        ...
        for n_id, n_data in nodes.items():
            ...
            if n_type == "UNIT":
                ...
                evidence_map[n_id] = {
                    ...
                    "reasoning_score": attrs.get("reasoning_score", 0.0),
                }
        ...
        for edge in edges:
            ...
            if src in topics_map and tgt in evidence_map:
                ...
                ev_item["similarity"] = float(edge.get("similarity", ev_item["similarity"]))
                ev_item["confidence"] = float(edge.get("confidence", ev_item["confidence"]))
                ev_item["reasoning_score"] = float(edge.get("reasoning_score", ev_item["reasoning_score"]))
        ...
        for t_id, t_data in topics_map.items():
            ...
            score = max([e["reasoning_score"] for e in ev_list], default=0.0)
            is_matched = ev_count > 0 and score >= self.confidence_threshold
            ...
            for e in ev_list:
                all_evidences_flat.append(
                    EvidenceDetail(
                        ...
                        validation_score=round(e["reasoning_score"], 4),
                    )
                )
        ...
        for u_id, u_data in units_map.items():
            ...
            u_covered_t = [
                t for t in u_topics
                if len(t["evidence"]) > 0
                and max([e["reasoning_score"] for e in t["evidence"]], default=0.0) >= self.confidence_threshold
            ]
            ...
            all_u_conf = [
                max([e["reasoning_score"] for e in t["evidence"]], default=0.0)
                for t in u_topics
            ]
            avg_u_conf = sum(all_u_conf) / len(all_u_conf) if all_u_conf else 0.0

            sig_score = sum(
                max([e["reasoning_score"] for e in t["evidence"]], default=0.0) * math.log(1 + len(t["evidence"]))
                for t in u_topics
            ) / max(1, tot_topics)
    ...
    def _build_visualizations(
        ...
        unit_bar = {
            ...
            "datasets": [{"label": "Coverage %", "data": [u.coverage_percentage for u in units]}],
        }
        ...
        for u_id, u_data in units_map.items():
            ...
            for t in u_topics:
                ...
                topic_children.append({
                    "name": t["label"],
                    "value": max(1, ev_cnt),
                    "confidence": max([e["reasoning_score"] for e in t["evidence"]], default=0.0),
                })

            node_data = {"name": u_data["label"], "children": topic_children}
